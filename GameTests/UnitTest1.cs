...
namespace CSharp_Course.GameTests
{
    ...
    {
        [Fact]
        public void TestGameHandling_PlayerWinBy5_WinResultPlayerScoreGet5PointsAndLifePointsStayMax()
        {
            // Arrange
            ...
            var result = gameHandler.HandleGame(3, 6, 1);
        ...
        public void TestPlayerLoot_PlayerUseLifePotionAndTrickDice_PlayerLifeIncreaseBy2PlayerDiceBy1AndWinResult()
        {
            ...
            // Act
            // Start fight with a dice result of 3 for player and 3 for enemy
            gameHandler.LootManager.LaunchTestLoot(gameHandler.Player.LootList);
            var result = gameHandler.HandleTestGame(3, 3, 3);

            // Assert
            // Check that result is tie because of the trick dice, monster type is average, score increase by 1 and player's life points increase by 2
            result.Should().Be(Result.Win);
            gameHandler._monsterFactory.WhichMonsterType().Should().Be(MonsterType.Average);
            gameHandler._weatherStation.WhichWeather().Should().Be(Weather.Sunny);
